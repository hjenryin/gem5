...
# Creates a (2-d torus) and directory controllers.
class Torus2D(SimpleTopology):
    description = "2D Torus"
    ...
    def makeTopology(self, options, network, IntLink, ExtLink, Router):
        ...
        side_length = round(math.sqrt(num_routers))
        print("side_length", side_length)
        assert side_length**2 == num_routers
        ...
        for i in range(num_routers):
            src_router = routers[i]
            dst_router = routers[(i + side_length) % num_routers]
            int_links.append(
                IntLink(
                    link_id=link_count,
                    src_node=src_router,
