...
def get_fn(cycle, traffic, injR, vc, topo, spin, node):
    return f"stats_{cycle}_{traffic}_injR{injR}_vc_{vc}_topo_{topo}_node_{node}{'_spin' if spin else ''}.txt"


dir_name = "deadlock-log/"


def get_cmd(cycle, traffic, injR, vc, topo, spin, node):
    if file_exists_and_not_empty(
        dir_name + get_fn(cycle, synthetic, injR, vc, topo, spin, node)
    ):
        return ""
    ...
    if topo == "Mesh_westfirst":
        ...
        routing = 1
    cmd = f'../build/NULL/gem5.fast -d "{dir_name}" -q --stats-file={get_fn(cycle,traffic,injR,vc,topo,spin,node)} ../configs/example/garnet_synth_traffic.py --network=garnet --num-cpus={node} --num-dirs={node} --topology={topo} --inj-vnet=0 --sim-cycles={cycle} --injectionrate={injR} --synthetic={traffic} --routing-algorithm {routing} --vcs-per-vnet={vc} {"--spin-enabled" if spin else ""} {topo_str} >/dev/null 2>&1'
    return cmd
...
for arr in files_list:
    cycle, synthetic, injR, vc, topo, spin = arr
    if topo == "Ring":
        node = 16
    else:
        node = 64
    cmd = get_cmd(cycle, synthetic, injR, vc, topo, spin, node)
...
for file in tqdm(files_list):
    cycle, synthetic, injR, vc, topo, spin = file
    if topo == "Ring":
        node = 16
    else:
        node = 64
    lat = 0
    fn = dir_name + get_fn(cycle, synthetic, injR, vc, topo, spin, node)
    with open(fn) as f:
        try:
    ...
    if lat == 0:
        ...
        lat = np.nan
        net_lat = np.nan
    arr = (cycle, synthetic, vc, topo, spin)
...
for net in traffic_list:
    for topo in topo_list:
        ...
        c = 0
        c = len(vc_list) * len(spin_list) * len(cycle_list) - 1
        max_hop = 0
        for vc in vc_list:
            for cycle in cycle_list:
                for spin in spin_list:
                    arr = (cycle, net, vc, topo, spin)
                    ...
                    ax1.plot(
                        injR_list[mask],
                        latencies[arr][mask],
                        label=f"{cycle} cycles, spin {'on' if spin else 'off'}",
                        color=color,
                    )
                    ax1.plot(
                        injR_list[mask],
                    ...
                    # Second subplot
                    ax2.plot(
                        injR_list[mask], throughputs[arr][mask], color=color
                    )
